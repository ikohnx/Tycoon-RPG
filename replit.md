...
- **Image-Based Tile System**: Complete image-based renderer using 1024x1024 PNG tile images
  - 59 tile types total: 35 base tiles + 24 industrial tiles (IDs 36-59)
  - All tiles loaded as PNG images from `/static/images/tiles/` via `drawImage()` with `imageSmoothingEnabled=false`
  - Animated overlay effects for water, portal, fountain, lamp, fireplace, waterfall, smokestack, gear
  - Fallback colored rectangles if images fail to load
  - Loading screen with progress bar waits for all tile/sprite images before showing title
  - Sprite rendering: 256x256 PNG character images rendered at TS+16 square (48x48 at SCALE=2), preserving aspect ratio
