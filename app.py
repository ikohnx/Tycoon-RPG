...
@app.route('/api/scenarios/<discipline>')
@login_required
...
@app.route('/api/buy/<int:item_id>', methods=['POST'])
@login_required
def api_buy(item_id):
    player_id = session.get('player_id')
    if not player_id:
...
@app.route('/api/play/<int:scenario_id>')
@login_required
...
@app.route('/api/choose/<int:scenario_id>/<choice>', methods=['POST'])
@login_required
